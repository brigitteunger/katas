...
class Solution():
    def merge_k_lists(self, lists: List[ListNode]) -> ListNode:
        ...
        index_smallest = self.find_smallest_node(lists)
        head = lists[index_smallest]

        if lists[index_smallest].next is None:
            del lists[index_smallest]
        else:
        ...
        while 1:
            if not lists:
                break
            index_smallest = self.find_smallest_node(lists)
            smallest_node = lists[index_smallest]
    ...
    def find_smallest_node(self, lists: List[ListNode]) -> int:
        smallest_value = lists[0].val
        index_smallest_node = 0
        for index in range(1, len(lists)):
            node = lists[index]
            if node.val < smallest_value:
                smallest_value = node.val
                index_smallest_node = index
        return index_smallest_node
